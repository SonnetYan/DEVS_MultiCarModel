...
public class trafficControlModel extends ViewableAtomic{
	double pf_maxElapseTimeWhenBusy_E2W = model_configData.pf_maxElapseTimeWhenBusy_E2W;
	double pf_maxElapseTimeWhenBusy_W2E = model_configData.pf_maxElapseTimeWhenBusy_W2E;
	double pf_carPassingTime_mean = model_configData.pf_carPassingTime_mean; // ---- this is used if treating car passing as a truncated normal distribution between low and high
...
// double lightSwitchTime_E2W;
// double lightSwitchTime_W2E;
...
public trafficControlModel(String name){
    this(name,"eastMoving_passive", DevsInterface.INFINITY, 
    		0, 0, 0, model_configData.pf_carPassingTime_mean, new Random(model_configData.randSeed_IntersectionM), model_configData.pf_maxElapseTimeWhenBusy_E2W, model_configData.pf_maxElapseTimeWhenBusy_W2E);
}

public trafficControlModel(String name,int QWestSideSize, int QEastSideSize){
    this(name,"eastMoving_passive", DevsInterface.INFINITY, 
    		0, QWestSideSize, QEastSideSize, model_configData.pf_carPassingTime_mean, new Random(model_configData.randSeed_IntersectionM), model_configData.pf_maxElapseTimeWhenBusy_E2W, model_configData.pf_maxElapseTimeWhenBusy_W2E);
}


public trafficControlModel(String name, String tfcState, double sgm, double initElapseTimeGreen, int QWestSideSize, int QEastSideSize, double carPassingTime, Random rd, double lSTime_E2W,double lSTime_W2E){
    super(name);
    ...
    pf_carPassingTime_mean = carPassingTime;
	pf_maxElapseTimeWhenBusy_E2W = lSTime_E2W;
	pf_maxElapseTimeWhenBusy_W2E = lSTime_W2E;
...
//	if(this.getSimulationTime()<600) {
//		carPassingTime_mean = model_configData.carPassingTime_mean;
//	}
//	else if(this.getSimulationTime()<1000) {
//		carPassingTime_mean = model_configData.carPassingTime_mean+0.005*(this.getSimulationTime()-600);
//	}
//	else if(this.getSimulationTime()<1800) {
//		carPassingTime_mean = model_configData.carPassingTime_mean+2.0-0.005*(this.getSimulationTime()-1000);
//	}
//	else {
//		carPassingTime_mean = model_configData.carPassingTime_mean;
//	}
//	System.out.println(this.getSimulationTime()+" carPassingTime_mean="+carPassingTime_mean);
...
protected void applyTrafficControlRules() {
	if(trafficLightState.startsWith("eastMoving")) { 							// eastMoving is green
		if(!queue_eastSide.isEmpty()&&(queue_westSide.isEmpty() 
				||elapsedTimeInGreen>= pf_maxElapseTimeWhenBusy_W2E )) 	// need to change signal
		...
		if(!queue_westSide.isEmpty() && (queue_eastSide.isEmpty()
				|| elapsedTimeInGreen>=pf_maxElapseTimeWhenBusy_E2W))	//need to change signal
...
public message  out( ){
	...
	if (phaseIs("eastMoving_active")) {
		...
		m.add(makeContent("report_out", new stateEntity(queue_westSide.size(),
				queue_eastSide.size(), phaseBeforeReporting, sigmaBeforeReporting, elapsedTimeInGreen, pf_carPassingTime_mean,pf_maxElapseTimeWhenBusy_E2W, pf_maxElapseTimeWhenBusy_W2E )));
	}
...
private double getCarPassingTime() {
	...
	double carPassingTime_LowBound = pf_carPassingTime_mean-model_configData.carPassingTime_rangeDelta; // 1 second ---- this is used if treating car passing as a truncated normal distribution between low and high
	double carPassingTime_UpBound = pf_carPassingTime_mean+model_configData.carPassingTime_rangeDelta; // 5 seconds ---- this is used if treating car passing as a truncated normal distribution between low and high
	nextTime = getTruncatedNormalDistributionRate(carPassingTime_LowBound, carPassingTime_UpBound,
			pf_carPassingTime_mean, carPassingTime_sigma);
