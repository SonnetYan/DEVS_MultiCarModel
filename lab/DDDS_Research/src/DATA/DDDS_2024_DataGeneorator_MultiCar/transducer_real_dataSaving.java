...
public class transducer_real_dataSaving extends ViewableAtomic{ 
	...
	double observationDuration = PF_configData.numberofsteps*observationInterval+2000;
...
//	int[] eastMovingArrivalVehicleIndex = new int[1000];
//	int[] westMovingArrivalVehicleIndex = new int[1000];
//	int eastMovingArrivalCurrentIdx=-1, westMovingArrivalCurrentIdx=-1;
	...
	public transducer_real_dataSaving(String nm){
		...
		try{
			eastMoving_input = new PrintWriter(new FileOutputStream(PF_configData.DataPathName+"eastMoving_input"+model_configData.run+".txt"), true);
			eastMoving_output = new PrintWriter(new FileOutputStream(PF_configData.DataPathName+"eastMoving_output"+model_configData.run+".txt"), true);
			westMoving_input = new PrintWriter(new FileOutputStream(PF_configData.DataPathName+"westMoving_input"+model_configData.run+".txt"), true);
			westMoving_output = new PrintWriter(new FileOutputStream(PF_configData.DataPathName+"westMoving_output"+model_configData.run+".txt"), true);
			observationData = new PrintWriter(new FileOutputStream(PF_configData.observedDataPathName+"observationData"+model_configData.run+".txt"), true);
			signalChangeTime = new PrintWriter(new FileOutputStream(PF_configData.DataPathName+"signalChangeTime"+model_configData.run+".txt"), true);
		}
...
//					eastMovingArrivalCurrentIdx = index;
					...
					int index = Integer.valueOf(ent.getName().substring(14,ent.getName().length()));
//					westMovingArrivalCurrentIdx = index;
					...
					if(!ent.getName().startsWith("eastMovingCar_init_")) {
						int index = Integer.valueOf(ent.getName().substring(14,ent.getName().length()));
					...
					if(!ent.getName().startsWith("westMovingCar_init_")) {
						int index = Integer.valueOf(ent.getName().substring(14,ent.getName().length()));
					...
					observationData.println(this.getSimulationTime()
							...
							+"\t"+ent.carPassingTime
							+"\t"+model_configData.maxElapseTimeWhenBusy_E2W
							+"\t"+model_configData.maxElapseTimeWhenBusy_W2E
							+"\t"+eastMoving_arrvCarCount
							+"\t"+westMoving_arrvCarCount
							+"\t"+eastMoving_dptCarCount
							+"\t"+westMoving_dptCarCount
